...
> **Note:** The Alloy plugin generates type definition files (`alloy-container.d.ts`, `alloy-manifests.d.ts`, and — when [custom scopes](/guide/scopes) are configured — `alloy-scopes.d.ts`) in your source directory (default: `./src`). Add these to your `.gitignore`.
